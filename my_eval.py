'''
COSC428 Real-time background subtraction on people with HMD project code
This is an extension/modification on https://github.com/dbolya/yolact/blob/master/eval.py. 
This program was modified so that it will display/write a real-time background subtraction video of VR players. 

Prerequisites:
- Anaconda is required to create an environment to run YOLACT real-time instance segmentation
- Download YOLACT project folder https://github.com/dbolya/yolact
- Run all the commands to complete installation written in the YOLACT repo README's installation section
- In the root of 'yolact' folder, create a folder named 'weights'
- Download a pretrained model yolact_base_54_800000.pth(https://drive.google.com/file/d/1UYy3dMapbH1BnmtZU4WH1zbYgOzzHHf_/view) and copy it to the weights folder
- Place this 'my_eval.py' to the 'yolact' root folder
- Place a video (only MP4 supported) you want to perform the background subtraction in the folder as well.

How to run this code:

1. Open a terminal/command prompt
2. Make sure to activate the conda environment you created in installation step by running 'conda activate yolact-env'
3. To display a real-time background subtraction, run the command: python my_eval.py --trained_model=weights/yolact_base_54_800000.pth --score_threshold=0.15 --top_k=15 --video=your-video.mp4
4. To write the video to a folder, specify the distination file name after the source video name separating them with ':'. The command is: python my_eval.py --trained_model=weights/yolact_base_54_800000.pth --score_threshold=0.15 --top_k=15 --video=your-video.mp4:new-video.mp4  
'''
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        #== COSC428 Background subtraction logic starts here ======= 
        # Some parta of code is studied and copied from https://github.com/MiguelAngeloMartinsRibeiro/YolactMiguel 
        
        # Draw a black frame as the base. On this frame, only human and HMD will be drawn by looping through all the instance detected in a frame
        base_image = np.zeros((h, w), np.uint8)
        
        for i in range(num_dets_to_consider):
            
            # Get classname of the mask instance
            class_name = cfg.dataset.class_names[classes[i]]
            
            # If the instance is part of VR player,
            if class_name in ["person", "cell phone"]:
                # Get the instance mask of an interest, has a type of torch.Tensor
                mask = masks[i, :, :, None]
                # Reshape the tensor
                mask = mask.view(1, mask.shape[0], mask.shape[1], 1)
                gpu_image = (mask.sum(dim=0) >=1).float().expand(-1, -1, 3)
                # Conver a GPU image to a CPU image and then to a numpy array frame
                numpy_image = (gpu_image * 255).byte().cpu().numpy()
                # Greyscale the image
                numpy_image = cv2.cvtColor(numpy_image, cv2.COLOR_RGB2GRAY)
                
                # Detect contours of the mask to get smoother mask
                contours, _ = cv2.findContours(numpy_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                
                # If contours were found, draw the mask using the contour
                if len(contours) is not 0:
                    contour = max(contours, key=cv2.contourArea)
                    # Initialize an all black frame
                    segment_image = np.zeros(numpy_image.shape, np.uint8)
                    # Draw contour and fill to create a mask
                    cv2.drawContours(segment_image, [contour], -1, 255, cv2.FILLED)
                    # Mask is drawn to the base_image to add up with other instance masks
                    base_image = cv2.bitwise_or(base_image, segment_image)

    # Original frame
    img_numpy = (img_gpu * 255).byte().cpu().numpy()
    
    # Subtrack the background using thresholding with the created mask
    res = cv2.bitwise_and(img_numpy, img_numpy, mask=base_image)
    return res
    #== End of COSC428 Background subtraction logic ======= 
